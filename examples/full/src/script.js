const app = {
  ...
  updateLoadingBar: function (percentage) {
    this.getElement("loading-bar", "#loading-bar").style.width =
      `${percentage * 100}%`;
    if (percentage === 1) {
  ...
  initLang: function () {
    ...
    this.getElement("lang-toggle", "#lang-toggle").addEventListener(
      "click",
      () => {
  ...
  initTheme: function () {
    ...
    this.getElement("theme-toggle", "#theme-toggle").addEventListener(
      "click",
      () => {
  ...
  setTheme: function (theme, force = false) {
    if (
      ...
      (theme !== this.theme || force)
    ) {
      this.theme = theme;
      localStorage.setItem("theme", theme);
      this.getElement("theme-toggle-text", "#theme-toggle p").innerText =
  ...
  setStyle: function () {
    ...
    manifestLink.href =
      manifestLink.attributes[`data-href-${this.theme}`].value;
    ...
    themeColorMeta.content =
      themeColorMeta.attributes[`data-content-${this.theme}`].value;
  ...
  initScrollAnchors: function () {
    ...
    if ("onscrollend" in window) {
      [
        window,
        ...this.getElements(
          "horizontal-anchors-containers",
          "[data-anchor-x-container]",
        ),
      ].forEach((element) => {
  ...
  calculateAnchorMap: function () {
    this.anchorMap.y = Array.from(
      this.getElements("vertical-anchors", "[data-anchor-y]"),
    ).map((element) => ({
    ...
    this.anchorMap.x = Array.from(
      this.getElements("horizontal-anchors", "[data-anchor-x]"),
    ).map((element) => ({
      parent: {
  ...
  setHash: function () {
    ...
    this.anchorMap.x.forEach((anchor) => {
      if (
        window.scrollY >= anchor.parent.start &&
        window.scrollY < anchor.parent.end &&
        anchor.parent.ref.scrollLeft >= anchor.start &&
        anchor.parent.ref.scrollLeft < anchor.end
      ) {
        console.log("setting hash", anchor.hash);
        history.pushState({}, "", `#${anchor.hash}`);
        // window.location.hash = anchor.hash;
  ...
  initIntroTextSwitcher: function () {
    ...
    const text = this.getElement(
      "intro-text-switcher-text",
      ".text",
      container,
    );
    const blinker = this.getElement(
      "intro-text-switcher-blinker",
      ".blinker",
      container,
    );
  ...
  initAboutScroll() {
    ...
    this.getElements("about-items", ".item", container).forEach(
      (item, index, array) => {
        const scrollLeft = this.getElement(
          `about-item-${index}-scroll-left`,
          "#scroll-left",
          item,
        );
        const scrollRight = this.getElement(
          `about-item-${index}-scroll-right`,
          "#scroll-right",
          item,
        );
        ...
        if (scrollLeft) {
          scrollLeft.addEventListener("click", () =>
            this.scroll(0, item.scrollWidth * (index - 1), container),
          );
        }
  ...
  initMisc: function () {
    this.getElement("current-year", "#current-year").innerText =
