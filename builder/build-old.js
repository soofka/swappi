...
const builder = {
  ...
  initTemplates: async function () {
    this.files.src.templates = await groupDirByFileType(
  ...
  initPartials: async function () {
    ...
    this.files.src.partials = groupDirByFileTypeRec(
      dir,
      FILES_GROUP_MAP,
      OTHER,
      2,
  ...
  parseImgFiles: async function () {
    ...
    for (let file of this.files.src.public.img) {
      ...
      for (let width of [CURRENT, ...this.config.options.optimize.img.widths]) {
        ...
        const distFileName = isCurrentWidth
          ? file.name
          : `${file.name}-${width}`;
        ...
        for (let type of [CURRENT, ...this.config.options.optimize.img.types]) {
          ...
          const distFileExists = this.files.dist.old.img.some(
            (oldFile) =>
              oldFile.name === newFile.name && oldFile.ext === newFile.ext,
          );
          ...
          if (
            ...
            if (isCurrentWidth && !isCurrentType) {
              ...
              newFile.content = await sharp(newFile.content)
                .resize(width)
                .toBuffer();
            } else if (!isCurrentWidth && !isCurrentType) {
              newFile.content = await sharp(newFile.content)
  ...
  parseHtmlFiles: async function () {
    for (let file of this.srcFiles.html) {
      ...
      for (let partial of this.files.src.partials.html) {
        const partialName = partial.name.substring(
          0,
          partial.name.lastIndexOf("."),
        );
          ...
          if (rule.type === "rule") {
            for (let declaration of rule.declarations) {
              if (declaration.type === "declaration") {
                const declarationName = declaration.value
                  .substring(1)
                  .split(":")[0];

                if (
                  declaration.property === CSS_PARTIAL_DECLARATION &&
                  declarationName === partialName
                ) {
                  declaration = partial.module(
                    declaration,
                    this.config,
                    this.distFiles,
                  );
  ...
  parseJsFiles: async function () {
    for (let file of this.srcFiles.js) {
      const fileContent = await readFile(file.full);
      const fileContentParsed = await minifyJs(
        fileContent,
        this.config.options.optimize.js,
      );
  ...
  checkIfFileUnchanged: function (group, file) {
    return this.files.dist.old[group].some(
      (oldFile) =>
  ...
  moveFileToDist: function (file) {
    ...
    const fileGroup =
      Object.keys(FILES_GROUP_MAP).find((key) =>
        FILES_GROUP_MAP[key].includes(file.ext),
      ) || OTHER;
